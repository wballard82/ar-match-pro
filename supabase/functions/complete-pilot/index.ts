...
Deno.serve(async (req) => {
  ...
  const { error: oe } = await p.a!.from("organizations")
    .update({ status: "pilot_completed", updated_by: p.caller!.userId }).eq("id", b.organization_id);
  if (oe) return fail(req, 409, "org_transition_failed", p.rid);
  const request_id = await recordEvent(p.a!, { event_type: "pilot_completed", actor: p.caller!.userId, org: b.organization_id as string });
  await p.a!.from("activation_events").insert({ organization_id: b.organization_id, event: "pilot_completed", actor_user_id: p.caller!.userId });
